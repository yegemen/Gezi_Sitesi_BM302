...
<div class="left">
    ...
    $id=@$_GET["id"];
    if (!empty($baglan)) {
        $v=$baglan->prepare("select * from gkonular inner join gkatagoriler on gkatagoriler.katagori_id=gkonular.konu_katagori where konu_katagori=?");
        $v->execute(array($id));
     ...
     if ($c){
         ...
         echo '<div class="">kategoriye ait konu bulunamadı </div>';
     }
